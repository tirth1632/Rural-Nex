...
export const userDataService = {
  async getCounts(): Promise<UserDataCounts> {
    try {
      const response = await apiFetch('/api/v1/users/data-counts/', {
        headers: {
  ...
  async exportUserData(): Promise<any> {
    try {
      const response = await apiFetch('/api/v1/users/export-data/', {
        headers: {
          'Content-Type': 'application/json',
          ...(localStorage.getItem('access_token')
            ? { Authorization: `Bearer ${localStorage.getItem('access_token')}` }
  ...
  async clearAssessmentHistory(): Promise<boolean> {
    try {
      const response = await apiFetch('/api/v1/users/clear-assessment-history/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...(localStorage.getItem('access_token')
  ...
  async deleteAllSavedData(): Promise<boolean> {
    try {
      const response = await apiFetch('/api/v1/users/delete-all-saved-data/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...(localStorage.getItem('access_token')
